...
export function buildProductBlendPrompt(
  ...
  const standardPrompt = [
    ...
    "首要任务：先判断背景属于方向光还是漫射光，再校正整个产品表面的亮面、暗面、高光、色温和环境染色，环境光影响必须清晰可见且自然。主光明确时形成符合产品曲面和光向的连续明暗与冷暖变化；漫射场景保持柔和层次，不虚构强方向光。不得仅通过整体压暗或提亮产品来表现融合。",
    "受光面应接受场景主光色，同时保持产品原有的中性基色和材质通透感；环境色只作用于合理的暗面、背光边缘和反射面，暗面和底部呈现合理的环境色与承载面反弹光，不得给整个产品统一染色。白色、黑色和金属表面的高光与反射必须服从场景光源；色彩变化必须平滑、通透，不得形成污渍、云斑、块状染色、颗粒、噪点或不规则涂抹。受光变化不是重新着色产品，仅增加地面阴影属于失败。",
    "主光明确时，投影方向必须与主光方向一致；漫射光下只生成紧凑的接触阴影、环境遮蔽和轻微地面反射。接触处最深，向外自然变软变淡，并随承载面的高度、凹凸和遮挡关系变形；不得形成均匀黑边或悬浮感。",
