...
export function buildProductBlendPrompt(
  ...
  const standardPrompt = [
    ...
    "首要任务：以第1张图背景为照明参考，校正整个产品表面的亮面、暗面、高光、色温和环境染色，使亮暗面、高光位置、环境色和反弹光与场景一致。环境光影响必须清晰可辨但克制自然，不能只增加地面阴影。",
    "环境色必须沿产品曲面连续、平滑过渡，不得形成污渍、云斑、块状染色、颗粒、噪点或不规则涂抹。白色产品只在合理的受光面、暗面和反射边缘接受克制、通透的环境色；黑色和金属表面的高光与反射也必须服从场景光源。受光变化不是重新着色产品，仅增加地面阴影属于失败。",
